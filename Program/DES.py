...
def initial_permutation(text):
    return permute(text, initial_permutation_table)

# Final permutation (FP) function
def final_permutation(text):
    return permute(text, final_permutation_table)

# Permutes the given text using the specified table
def permute(text, table):
    return ''.join(text[i - 1] for i in table)

# Function to expand the text to 48 bits
def expand(text):
    return permute(text, expansion_table)

# Function to apply the S-boxes to the given text
def s_box(text):
    output = ""
    for i in range(8):
...
def p_permutation(text):
    return permute(text, p_box)
...
def generate_round_keys(key):
    ...
    for i in range(16):
        left = left_shift(left, shift_amounts[i])
        right = left_shift(right, shift_amounts[i])
        keys.append(permute(left + right, key_compression_table))
...
# Key compression table for generating round keys
...
def des_encrypt(plaintext, key):
    ...
    for i in range(16):
        ...
        xor_result = bin(int(right_expanded, 2) ^ int(keys[i], 2))[2:].zfill(48)
        after_sbox = s_box(xor_result)
        after_permutation = p_permutation(after_sbox)
        new_right = bin(int(left, 2) ^ int(after_permutation, 2))[2:].zfill(32)
        left = right
        right = new_right

    cipher_text = final_permutation(right + left)
    return cipher_text
...
def des_decrypt(ciphertext, key):
    ...
    plain_text = final_permutation(right + left)
    return plain_text
